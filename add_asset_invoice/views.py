...
def login_view(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            #return redirect(request,'index', context)  # قم بتوجيه المستخدم إلى الصفحة التي ترغب فيها بعد تسجيل الدخول
...
class LogoutView(View):
    def get(self, request):
        logout(request)
        return redirect('loginn')  # يمكنك توجيه المستخدم إلى أي صفحة تريدها بعد تسجيل الخروج
...
def index(request):
    ...
    if request.user.is_authenticated:
        # إذا كان المستخدم مسجل دخوله، قم بالحصول على اسم المستخدم
